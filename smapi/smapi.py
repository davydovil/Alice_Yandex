...
class Client(APIBase):
    ...
    def get_token(self, login, password):
        session = Session()
        data = {"exceededAttempts": "False", "ReturnUrl": "", "login": login, "password": password, "Captcha.Input": "",
                "Captcha.Id": ""}
        session.post("https://login.school.mosreg.ru/login", data=data)
        token = session.get(
            f'https://login.school.mosreg.ru/oauth2?response_type=token&client_id=bafe713c96a342b194d040392cadf82b&scope=CommonInfo,ContactInfo,FriendsAndRelatives,EducationalInfo,SocialInfo&redirect_uri=').url
    ...
    def get_folder_files(self, folder_id, page_number=1, page_size=10, order_by='id_asc'):
        return self.get(f'folder/{folder_id}/files',
                        params={'page_number': page_number, 'page_size': page_size, 'order_by': order_by})
    ...
    def get_school_homework_for_date_period(self, school_id, start_date, end_date):
        return self.get(f'users/me/school/{school_id}/homeworks?startDate={start_date}&endDate={end_date}')
    ...
    def get_person_school_homework_for_date_period(self, person_id, school_id, start_date, end_date):
        return self.get(f'persons/{person_id}/school/{school_id}/homeworks?startDate={start_date}&endDate={end_date}')
    ...
    def get_group_lesson_log_by_subject_for_date_period(self, group_id, subject_id, from_date, to_date):
        return self.get(f'lesson-log-entries/group/{group_id}',
                        params={'subject': subject_id, 'from': from_date, 'to': to_date})

    def get_person_lesson_log_by_subject_for_date_period(self, person_id, subject_id, from_date, to_date):
        return self.get(f'lesson-log-entries/person/{person_id}/subject/{subject_id}/{from_date}/{to_date}')

    def get_person_lesson_log_for_date_period(self, person_id, start_date, end_date):
        return self.get(f'persons/{person_id}/lesson-log-entries/{start_date}/{end_date}')
    ...
    def get_person_schedule_by_group_for_date_period(self, person_id, group_id, start_date, end_date):
        return self.get(f'persons/{person_id}/groups/{group_id}/shcedules',
    ...
    def get_person_tasks_by_subject_for_date_period(self, person_id, subject_id, from_date, end_date, page_number=1,
                                                    page_size=10):
        return self.get(f'persons/{person_id}/tasks',
                        params={'subject': subject_id, 'fromDate': from_date, 'endDate': end_date,
    ...
    def get_person_group_thematic_marks_by_subject_for_date_period(self, person_id, group_id, subject_id, from_date,
                                                                   to_date):
        return self.get(
            f'persons/{person_id}/edu-groups/{group_id}/subjects/{subject_id}/thematic-mmarks/{from_date}/{to_date}')
